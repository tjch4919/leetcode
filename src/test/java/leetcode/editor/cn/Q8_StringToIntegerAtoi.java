...
public class Q8_StringToIntegerAtoi{
    ...
    class Solution {
        public int myAtoi(String s) {
            ...
            boolean startNumber = false, negative = false;
            int result = 0;
            for (char aChar : chars) {
                if (aChar == ' ') {
                    ...
                    startNumber = true;
                    // 这个方式比Integer.parseInt(String.valueOf(aChar)) 快很多
                    int currentNumber = aChar - '0';
                    if (!negative && (result > positiveMaxComp || (result == positiveMaxComp && currentNumber > positiveMaxRest))) {
                        return Integer.MAX_VALUE;
                    } else if (negative && (-result < negativeMaxComp || (-result == negativeMaxComp && -currentNumber < negativeMAxRest))) {
                        return Integer.MIN_VALUE;
                    } else {
                        result = 10 * result + currentNumber;
                    }
                } else {
                    break;
                }
